...
CREATE TABLE public.book_issues (
    ...
    CONSTRAINT book_issues_status_check CHECK (((status)::text = ANY ((ARRAY['ISSUED'::character varying, 'RETURNED'::character varying, 'OVERDUE'::character varying])::text[])))
);
...
CREATE TABLE public.users (
    ...
    CONSTRAINT users_role_check CHECK (((role)::text = ANY ((ARRAY['ADMIN'::character varying, 'STAFF'::character varying, 'GUEST'::character varying])::text[])))
);
